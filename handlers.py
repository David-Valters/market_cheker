...
async def add_legendary_skins():
    ...
    for count, item in enumerate(legendary_skin):
        ...
        db.add_skin(
            skin_id,
            item.get("name", "Unknown Skin"),
            price,
            make_url_icon(item.get("iconUrl", "")),
        )
        logger.info(
            f"Done {count+1}/{len(legendary_skin)}: {item['name']} ({skin_id}) - {price}"
        )
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔍 Вибрати скін", switch_inline_query_current_chat=""
                )
            ],
        ]
    )
...
async def on_selected_result(message: types.Message):
    logger.info(
        f"[SKIN COMMAND] Name: {message.chat.full_name}, ChatId: {message.chat.id}, text: '{message.text}'"
    )
    ...
    if is_in_db:
        second_button = InlineKeyboardButton(
            text="❌ Видалити з обраного", callback_data=f"del:{selected_id}"
        )
    else:
        second_button = InlineKeyboardButton(
            text="➕ Додати до обраного", callback_data=f"add:{selected_id}"
        )
...
async def inline_handler(query: InlineQuery):
    ...
    logger.info(
        f"[INLINE QUERY] From: {query.from_user.username}, query: '{user_query}'"
    )
    if not user_query:
        results = [
        ...
        data_filter = [
            item for item in data if user_query.lower() in item["name"].lower()
        ]
        results = [
            InlineQueryResultArticle(
                id=str(uuid.uuid4()),
...
async def handle_add(callback: CallbackQuery):
    logger.info(
        f"[CALLBACK ADD] From: {callback.from_user.username}, data: '{callback.data}'"
    )
    ...
    if not data_element:
        ...
        db.add_skin(
            selected_id,
            data_element.get("name", "Unknown Skin"),
            price,
...
async def handle_del(callback: CallbackQuery):
    logger.info(
        f"[CALLBACK DEL] From: {callback.from_user.username}, data: '{callback.data}'"
    )
...
async def handle_info(callback: CallbackQuery):
    logger.info(
        f"[CALLBACK INFO] From: {callback.from_user.username}, data: '{callback.data}'"
    )
...
async def send_logs(message: types.Message):
    # Захист: лише для адміністратора
    logger.info(f"[LOG] Name: {message.chat.full_name}, ChatId: {message.chat.id}")
    ...
    log_file = FSInputFile(log_path, filename="app.log")
